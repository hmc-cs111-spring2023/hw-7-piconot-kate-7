...
object PiconotParser extends JavaTokenParsers {
    ...
    def rule: Parser[Rule] =
        ...
        | "when the"  ~> dir  ~ ("road ends, get off route" ~> state ~ ("and turn" ~> dir ~ ("onto route" ~> state))) ^^ {
            case d_old ~ (s_old ~  (d_new ~  s_new)) => Rule(s_old, RoadClosed(d_new, d_old), d_new, s_new) } // changing state And direction because ahead direction is blocked
        | "when the" ~> dir ~ ("road ends, turn" ~> dir ~ ("to continue on route" ~> state)) ^^ {
            case d_old ~ (d_new ~  s) => Rule(s, RoadClosed(d_new, d_old), d_new, s) } // changing direction but not state
        | "when the" ~> dir ~ ("road ends," ~> dir ~ ("to continue on route" ~> state)) ^^ {
            case d_old ~ (stop ~ s) => Rule(s, RoadEnds(d_old), stop, s) } // not moving, nor changing state
        | "when the" ~> dir ~ ("road ends," ~> dir ~("to get off route" ~> state ~ ("and turn" ~> dir ~ ("onto route" ~> state)))) ^^ {
            case d_old ~ (stop ~ (s_old ~ (d_new ~ s_new))) => Rule(s_old, RoadClosed(d_new, d_old), stop, s_new) } // not moving, but yes changing state
        ...
        | "when the" ~> dir ~ ("road ends, and the" ~> dir ~ ("road is closed," ~> dir ~ ("to continue on route" ~> state))) ^^ {
            case d_old ~ (d_closed ~ (stop ~ s)) => Rule(s, TwoRoadsClosed(d_old, d_closed), stop, s) } // not moving, nor changing state, two road closures
        |  "when the" ~> dir ~ ("road ends, and the" ~> dir ~ ("road is closed," ~> dir ~ ("to get off route" ~> state ~ ("and turn" ~> dir ~ ("onto route" ~> state))))) ^^ {
            case d_old ~ (d_closed ~ (stop ~ (s_old ~ (d_new ~ s_new)))) => Rule(s_old, TwoClosedOneOpen(d_new, d_old, d_closed), stop, s_new) } // not moving, but yes changing state
        ...
        | "when the" ~> dir ~ ("road ends, and the" ~> dir ~ ("and" ~> dir ~ ("roads are closed," ~> dir ~ ("to continue on route" ~> state)))) ^^ {
            case d_old ~ (d_closed ~ (d_closed_2 ~ (stop ~ s))) => Rule(s, StayThreeRoads(d_old, d_closed, d_closed_2), stop, s) } // not moving, nor changing state, three road closures
        | "when the" ~> dir ~ ("road ends, and the" ~> dir ~ ("and" ~> dir ~ ("roads are closed," ~> dir ~ ("to get off route" ~> state ~ ("and turn" ~> dir ~ ("onto route" ~> state)))))) ^^ {
            case d_old ~ (d_closed ~ (d_closed_2 ~ (stop ~ (s_old ~ (d_new ~ s_new))))) => Rule(s_old, StayThreeRoads(d_old, d_closed, d_closed_2), stop, s_new) } )// not moving, but yes changing state
